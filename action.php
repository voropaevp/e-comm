                        ...
                        <div class='col-md-4 col-xs-6' >
								<a href='product.php?p=$pro_id'><div class='product'>
									...
									<div class='product-body'>
										...
										<button type='button' class='btn btn-primary'><span>$</span>$pro_price</button>
										</div>
...
if(isset($_POST["addProductReview"])){
		$p_id = $_POST["proId"];
		$cus_review = $_POST["cus_review"];
		$cus_name = $_POST["cus_name"];
		$cus_email = $_POST["cus_email"];
		$star = $_POST["star"];
$sql = "INSERT INTO `tbl_rating`(`product_id`, `name`, `email`, `review`, `rating`)
VALUES ('$p_id','$cus_name','$cus_email','$cus_review','$star')";
	...
	if(isset($_POST["addToCarts"])){
		...
		$count = mysqli_num_rows($run_query);
		if($count > 0){
						$sql = "Update `cart` set `ip_add`='$ip_add', `qty`='$p_qty'
			WHERE `user_id`='$user_id' AND `p_id`='$p_id'";

		} else {
			$sql = "INSERT INTO `cart`
			(`p_id`, `ip_add`, `user_id`, `qty`)
			VALUES ('$p_id','$ip_add','$user_id','$p_qty')";
						...
						<tr>
							<td data-th="Product" >
								<div class="row">
									...
									<h4 class="nomargin product-name header-cart-item-name">'.$product_title.'</h4></a>
									</div>
									<div class="col-sm-6">
