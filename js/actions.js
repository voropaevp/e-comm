...
//Add Product into Cart
	$("body").delegate("#products","click",function(event){
		...
		$.ajax({
			url : "action.php",
			method : "POST",
			data : {addToCarts:1,proId:pid,Pqty:qty},
			success : function(data){
				count_item();
				getCartItem();
	...
	*/
    ...
    $("body").delegate("#review","click",function(event){
       event.preventDefault();
      var star= $("input[name='rating']:checked").val();
      var cus_name= $("#cus_name").val();
      var cus_email= $("#cus_email").val();
      var cus_review= $("#cus_review").val();
      var proId=$("#Pid").val();
       //alert(star+''+cus_name+''+cus_email+''+cus_review);
        $.ajax({
            url	:	"action.php",
            method	:	"POST",
            data	:	{addProductReview:1,proId:proId,cus_name:cus_name,cus_email:cus_email,cus_review,star},
            success	:	function(data){
                window.location.href = "product.php?p="+proId+"&review=add";
                //$("#cart_msg").html(data);
               // checkOutDetails();
                }
            })
